...
<script lang="ts">
import { defineComponent, ref, onMounted, computed, defineProps, watch } from "vue"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const fetchDataDeltasBoidID = async() => {
      if (selectedBoidId.value && props.fromDate && props.toDate) {
        ...
        const ensureFiniteNumber = (obj:DeltasDataItem, property:string) => {
          let val:string | number = obj[property]
    ...
    return {
      manualFetch,
      boidIDstakePowerSeriesFinal,
      boidIDstakePowerOptionsFinal,
      combinedDataSeriesFinal,
      combinedDataOptionsFinal,
      logpowerclaimOptionsFinal,
      logpowerclaimSeriesFinal,
      mintedBoidIDOptionsFinal,
      mintedBoidIDSeriesFinal
    }
  }
})
</script>
